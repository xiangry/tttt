...
local uiInformation = {};
uiInformation[EUI.BattleUI] = { btnBack = false, background = false, showLast = false, };

-------------------------导航条栏目图片标题(没有设置则不显示)---------------
uiInformation[EUI.BattleUI].sp_title = "title_1"
uiInformation[EUI.BattleUI].coin = { id = 21, get_way = false, }
------------------------ 隐藏 导航条（默认0全部显示 1隐藏 2隐藏三个货币栏） ----------------------
uiInformation[EUI.BattleUI].showState = 1
------------------------ 导航条公用底图（默认底图 ENUM.PublicBgImage.DLD 可不配置） ----------------------
uiInformation[EUI.BattleUI].resBg = ""
------------------------ ui界面的背景音乐，不配置的话就不改变 ----------------------
uiInformation[EUI.BattleUI].backAudioId = ENUM.EUiAudioBGM.MainCityBgm;
