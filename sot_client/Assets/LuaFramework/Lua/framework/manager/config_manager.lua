...
function ConfigManager.SpliceIndexName(name)
	if EConfigIndex['t_'..tostring(name)] == nil then
		if not AppConfig.get_is_numerical_version() then
			app.log_error("can't found index name="..tostring(name).." "..debug.traceback())
		end
	end
...
--	util.begin_sample("config_get")
	...
	--pp.log("xxx ConfigManager.Get configname="..configName.." dataID"..dataID)
    ...
    if nil == r then
    	-- util.end_sample();
    	-- app.log_error("configName:" .. configName .. " id: " .. dataID .. debug.traceback())
    	return nil
	end
...
function ConfigManager._GetConfigTable(configName)
	-- app.log_error("active config table :" .. configName)

	ConfigManager.__ActiveConfigTable(configName);
	-- app.log("<color=aqua> 技能配置表name </color>"..tostring(configName))
	local cfg = ConfigManager.__get_config(configName)
	...
	-- 	if cnt > 100 then
	-- 		app.log_error("acitve big table:" .. configName)
	-- 	end
	-- end
...
function ConfigManager.__ActiveConfigTable(configName)
	...
	if nil == cfg then
		app.log_error(string.format("__ActiveConfigTable failed: %s not found.", tostring(configName)));
		return false;
	end

	-- if nil == cfg.query_func then
	-- 	app.log_error(string.format("__ActiveConfigTable failed. %s query func is nil.", tostring(configName)));
	-- 	return false;
	-- end
	...
	--app.log_error(configName.."....."..table.tostring(cfg.table)..debug.traceback())
...
function ConfigManager._AppendRow(configName, key, data, _show_replace_warning_msg)
	...
	if nil == key or nil == data or nil == cfg or nil == cfg.table then
		app.log_error(string.format("_AppendRow failed. configName: %s, key:%s, data: %s", tostring(configName), tostring(key), tostring(data)) )
		return false
	end
...
function ConfigManager.LoadIndex()
	...
	if nil == EConfigIndex or nil == g_ConfigDetails then
		app.log_error("config index file error.");
		return false;
	end
	return true;
end
...
function ConfigManager.LoadConfig(configName)
	...
	if nil == cfgFileInf then
		app.log_error("config file not found:" .. tostring(configName).. debug.traceback());
		return false 
	end
	...
	--app.log_error("......."..tostring(configFilePath))
	...
	if cfgFileInf.table_name then
		...
		if nil == loadedTable then
			app.log_error("config file load error:" .. configName .. ":".. table.tostring(cfgFileInf.table_name)..debug.traceback())
			return false
		end
		...
		if nil ~= mt then
			app.log_error("配置表已经存在metatable:%s"..configName)
		end
