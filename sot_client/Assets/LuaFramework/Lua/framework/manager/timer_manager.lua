...
function TimerManager.Add( Callback, Interval, Times, Data, Weight )
	...
	-- app.log("<color=aqua> 我走了==TimerManager.Add== </color>"..tostring(Data))
	...
	Timer.Times		= Times;
	Timer.Data		= Data;
	Timer.BeginTime	= app.get_time() * 1000; -- PublicFunc.QueryFightCurTime();
...
function TimerManager.Execute()
	local Now = app.get_time() * 1000; -- PublicFunc.QueryFightCurTime();
			...
			if Timer.Callback ~= nil then
				...
				if func == nil then
					...
					if Timer.DebugMsg then
						app.log_error("timer callback被移除掉但未清除定时器: "..tostring(Timer.DebugMsg))
					end
				else
