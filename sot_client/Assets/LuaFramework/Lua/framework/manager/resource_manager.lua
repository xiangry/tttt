...
local function __is_main_city_reserved_ui(path)
    ...
    -- if nil == _fm then
    --     -- app.log_error("__is_main_city_reserved_ui  nil fightmanager........." );
    --     return false;
    -- end
...
function ResourceManager.AddResourceObject(path, obj)
    ...
    if __is_main_city_reserved_ui(path) then
        -- app.log_error("add reserved res:" .. path);
...
function ResourceManager.SetControlHeroReservedRes(pos, data)
    ...
    -- app.log_error(">>>>>>>>>> add hero reserved res:"..tostring(pos).." ,"..tostring(#data))
    for k, v in pairs(data) do
        -- app.log_error("add control hero reserved res ..".. v)
...
function ResourceManager.IsReservedRes(path)
    ...
    --TODO:kevin 临时不释放所ui
    -- app.log_error("xxxxxxxxxxxxx:"..path)
    if ResourceManager.main_city_reserved_ui_res[path] ~= nil then
        -- app.log_error("find reserved res:" .. path)
        return true;
    end
