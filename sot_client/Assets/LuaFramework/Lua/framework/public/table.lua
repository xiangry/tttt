...
local _mt = {
	__newindex = function(t, k, v) app.log_error("试图写入table.empty "..debug.traceback()) end
}
...
function table.tostring(t,_n)
	...
	function _t:_tostring(t,n)
		...
		str[#str+1] = '{\n'
		if type(t) ~= 'table' then
			app.log_error('打印表传入的不是表。。'..debug.traceback())
		end
		for k,v in pairs(t) do
...
function table.getall(data)
	if data == nil 
		then 
		app.log("table.getall data is nil");
	end 
	if(type(data)=="table")
		then
		local num = 0;
...
function table.get_value_num(t)
	if type(t) ~= 'table' then
        return 0
    end
    local num = 0
