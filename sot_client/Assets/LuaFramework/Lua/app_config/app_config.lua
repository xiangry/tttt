...
Target_Version = Target_Version_Name.ART
---
AppConfig = {
	--------------------------- [[打包 start]]-------------------------------------------------------------------------------
	is_release = false; --[[ ********是不是发布版本******* ]]
	is_digisky_sdk = true;--[[ ********是不是使用公司SDK版本******* ]]
	package_version1 = "0"; --[[版本号1]]
	package_version2 = "0"; --[[版本号2]]
	package_version3 = "1"; --[[版本号3（针对线上:如果公共资源有改变就需要改变版本号）]]
	...
	----------------------------- [[系统级主要参数]]----------------------------------------------------------------------------------------
	is_numerical_version = false; --[[数值测试版本]]
	new_player_script_recording = false;	  --[[新玩家消息脚本录制]]
	update_region = 1; --[[ 语言区域版本 1:中文 ]]
	enable_peak_show = false; --[[ 巅峰展示   true启用   false关掉 ]]
	enable_guide = false; --[[ 新手引导   true启用   false关掉 ]]
	enable_game_cg = false; --[[ 开场视频+剧情   true启用   false关掉 ]]
	enable_guide_tip = false; --[[ 引导提示   true启用   false关掉 ]]
	enable_tip = true; --[[ 界面提示   true启用   false关掉 ]]
	enable_on_line = false; --[[ 线上流程   true启用  false关掉 ]]
	enable_update_file = false; --[[ 文件动态更新   true启用  false关掉 ]]
	enable_keep_alive_check = true; --[[ 客户端主动心跳，SOCKET超时检查 ]]
	enable_reg_mail = true;--[[公司平台帐号邮箱注册 true启用  false关掉]]
	enable_queue = false; --[[ 官网排队   true启用  false关掉 ]]
	...
	is_web_post = true; --[[ 向WEB提交信息 true提交 ]]
	is_debug_show = true; --[[ 显示debug按钮 true显示 ]]
	is_debug_check = true; --[[ debug按钮点击事件 true接受点击 ]]
...
--			client_version = ""; --[[ 评审服标示 ：平台DTREE client_version的值]]
	...
	http = {
		...
		art = {
			check_op =
			{
				[1] = "http://leiowy.f3322.net:7000/http_update/",
				--[2] = "http://125.71.203.241:9321/http_update/",
			};
			down_url = {
				[1] = "http://leiowy.f3322.net:7000/http_update/",
			...
			update_device_id = "update_1"; --[[文件更新目录]]
			update_version_number = 0; --[[ 初始版本号(没有任何资源的版本号) ]]
		};
	};
	--------------------------- [[自己写的HTTP更新 end]]---------------------------------------------------------------------------------
...
if Target_Version == Target_Version_Name.BETA then
	...
	AppConfig.enable_tip = true;--[[ 界面提示   true启用   false关掉 ]]
	AppConfig.enable_on_line = false; --[[ 线上流程   true启用  false关掉 ]]
	AppConfig.enable_update_file = true; --[[ 文件动态更新   true启用  false关掉 ]]
	AppConfig.enable_keep_alive_check = true; --[[ 客户端主动心跳，SOCKET超时检查 ]]
	AppConfig.enable_reg_mail = true; --[[公司平台帐号邮箱注册 true启用  false关掉]]
...
if Target_Version == Target_Version_Name.QA then
	...
	--[[配置全从服务器下载，本地配置无效]]
	AppConfig.enable_peak_show = false; --[[ 巅峰展示   true启用   false关掉 ]]
	AppConfig.enable_guide = false; --[[ 新手引导   true启用   false关掉 ]]
	AppConfig.enable_guide_tip = true; --[[ 引导提示   true启用   false关掉 ]]
	AppConfig.enable_tip = true;--[[ 界面提示   true启用   false关掉 ]]
	AppConfig.enable_on_line = true; --[[ 线上流程   true启用  false关掉 ]]
	...
	--[[android]]
	AppConfig.system_list.android.system_check_httpurl = {
		[1] = "http://cn.web.gh.ppgame.com:2601/";--[[QA服]]
	};
	AppConfig.system_list.android.system_check_filepath = "entry_service/entry_info.do?channel_id=%s&client_ver=%s&package_id=%s"; --[[ 入口HTTP PATH ]]
	AppConfig.system_list.android.effect_level_filepath = "entry_service/effect_level.do?device_model=%s";--[[特效等级PATH]]
	AppConfig.system_list.android.notice_info_path = "entry_service/notice_info.do";--[[维护公告PATH]]
...
--	AppConfig.system_list.android.client_version = ""; --[[ 评审服标示 ：平台DTREE client_version的值]]
	...
	AppConfig.system_list.android.fserver_version_number = 11366;--[[fserver更新起始版本号]]
	--[[IOS]]
	AppConfig.system_list.ios.system_check_httpurl = {
		[1] = "http://cn.web.gh.ppgame.com:2601/";--[[QA服]]
	};
...
--	AppConfig.system_list.ios.client_version = ""; --[[ 评审服标示 ：平台DTREE client_version的值]]
	...
	AppConfig.system_list.ios.dtree_channle_id = "1999";--[[ DTREE的channel,如果不为空就以这个值去取DTREE，如果为空就以上面的真channel取去 进入GAME服也要传这个值 混服使用]]
	AppConfig.system_list.ios.fserver_version_number = 1882;--[[fserver更新起始版本号]]
end
...
if AppConfig.is_release or Target_Version == Target_Version_Name.TEST_ONLINE then
	...
	AppConfig.is_digisky_sdk = true;--[[ ********是不是使用公司SDK版本******* ]]
	...
	--[[android]]
	AppConfig.system_list.android.system_check_httpurl = {
		[1] = "http://entry.ghoul.ppgame.com:2601/";
	};
	AppConfig.system_list.android.system_check_filepath = "entry_service/entry_info.do?channel_id=%s&client_ver=%s&package_id=%s"; --[[ 入口HTTP PATH ]]
	AppConfig.system_list.android.effect_level_filepath = "entry_service/effect_level.do?device_model=%s";--[[特效等级PATH]]
	AppConfig.system_list.android.notice_info_path = "entry_service/notice_info.do";--[[维护公告PATH]]
...
--	AppConfig.system_list.android.client_version = ""; --[[ 评审服标示 ：平台DTREE client_version的值]]
	...
	AppConfig.system_list.android.fserver_version_number = 13631;--[[fserver更新起始版本号]]

	--[[IOS]]
	AppConfig.system_list.ios.system_check_httpurl = {
		[1] = "http://ghoul-ios-cn-ps-login-001.ppgame.com:8080/";
	};
	AppConfig.system_list.ios.system_check_filepath = "entry_service/entry_info.do?channel_id=%s&client_ver=%s&package_id=%s"; --[[ 入口HTTP PATH ]]
	AppConfig.system_list.ios.effect_level_filepath = "entry_service/effect_level.do?device_model=%s";--[[特效等级PATH]]
	AppConfig.system_list.ios.notice_info_path = "entry_service/notice_info.do";--[[维护公告PATH]]
...
--	AppConfig.system_list.ios.client_version = ""; --[[ 评审服标示 ：平台DTREE client_version的值]]
	...
	AppConfig.system_list.ios.fserver_version_number = 7383;--[[fserver更新起始版本号]]
	...
	if Target_Version == Target_Version_Name.TEST_ONLINE then
		...
		AppConfig.system_list.android.dtree_channle_id = "1007";--[[ DTREE的channel,如果不为空就以这个值去取DTREE，如果为空就以上面的真channel取去]]

		--[[IOS]]
		AppConfig.system_list.ios.system_check_httpurl = {
			[1] = "http://ghoul-ios-cn-pre-login-0001.ppgame.com:8080/" --[[预发布服]]
		};
		AppConfig.system_list.ios.dtree_channle_id = "2000";--[[ DTREE的channel,如果不为空就以这个值去取DTREE，如果为空就以上面的真channel取去]]
	end
...
--------------------------------- [[通用]]----------------------------------------
function AppConfig.get_is_release()
...
--[[ 是否跳过界面提示 ]]
function AppConfig.get_enable_tip()
	if Socket.get_socketgame() then
		return AppConfig.enable_tip;
	else
		return false
	end
end
...
function AppConfig.get_check_appstore()
	if AppConfig.get_platformchannel_id() == "2001" and Root.get_os_type() == 8 then
		return true;
	else
		return false;
	end
...
function AppConfig.get_check_digisky_channel()
	if AppConfig.get_platformchannel_id() == "1007" or AppConfig.get_platformchannel_id() == "1008" or AppConfig.get_platformchannel_id() == "2001" then
		return true;
	else
		return false;
	end
...
function AppConfig.get_digisky_queue_url()
	if AppConfig.get_check_digisky_channel() then
		return AppConfig.get_account_url();
--		return AppConfig.digisky_queue_url;--[[OLD]]
	else
		return "";
	end
end
...
function AppConfig.get_digisky_queue_path()
	if AppConfig.get_check_digisky_channel() then
		return AppConfig.get_auth_server_path();
--		return AppConfig.digisky_queue_path;--[[OLD]]
	else
		return "";
	end
end
