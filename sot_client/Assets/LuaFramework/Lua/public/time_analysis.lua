...
--精确到分钟简化显示
function TimeAnalysis.analysisSec_to_m(sec, showBefore, showExact)
	local timeStr = {}
	if(sec >= 24*60*60) then
		local day = math.floor(sec/(24*3600))
		sec = sec - day*24*3600
	...
	if(sec > 60*60) then
		local hour = math.floor(sec/(3600))
		sec = sec - hour*3600
		table.insert(timeStr, hour..'时')
	end
	if(sec > 60) then
		local min = math.floor(sec/(60))
		sec = sec - min*60
		table.insert(timeStr, min..'分')
	end
	--不到1分钟的
	if #timeStr == 0 then
		if showExact then
...
function TimeAnalysis.FormatTimeS(time)
    local time_str = time .. ""
    if string.len(time_str) == 2 then
        return time_str
    end
    if string.len(time_str) == 3 then
        return string.sub(time_str, 1,1) .. "''" .. string.sub(time_str, 2, 3)
    end
    return string.sub(time_str, 1, 2) .. "''" .. string.sub(time_str, 3, 4) 
end
...
function TimeAnalysis.TableToTimeSecond(t)
    ...
    local tTime = {year = t.year, month = t.month, day = t.day, hour = t.hour, min = t.min, sec = t.sec}
