...
function algorthm.GetDistanceSquared(x1, z1, x2, z2)
	if nil == x1 or nil == z1 or nil == x2 or nil == z2 then
		app.log_error('GetDistanceSquared x1='..tostring(x1)..' z1='..tostring(z1)..' x2='..tostring(x2)..' z2='..tostring(z2)..' '..debug.traceback())
	end
	local dx = x2 - x1;
	local dz = z2 - z1;
	local squaredDis = dx * dx + dz * dz;
...
function algorthm.GetDistance(x1, z1, x2, z2)
    if nil == x1 or nil == z1 or nil == x2 or nil == z2 then
        app.log_error('algorthm.GetDistance' .. debug.traceback())
    end
...
function algorthm.GetSpaceDistanceSquared(x1, y1, z1, x2, y2, z2)
    local dx = x1-x2
    local dy = y1-y2
    local dz = z1-z2
...
----------------------------------返回角色列表----------------------------------------------
local sort_overlap_sphere = function(a, b)
...
function algorthm.GetOverlapSphere(pos, radius, dir, angle, layer, targetRadius)
    ...
    if pos == nil then
        app.log_error("OverlapSphere pos为nil stack=" .. debug.traceback())
        return nil
    end
    local new_radius = radius + PublicStruct.Const.SEARCH_OBJ_RADIUS_OFFSET
    local obj_list
    if PublicStruct.Fight_Sync_Type == ENUM.EFightSyncType.Replay then
        obj_list = ObjectManager.OverlapSphere(pos.x, pos.y, pos.z, new_radius, layer);
    else
        obj_list = util.overlap_sphere(pos.x, pos.y, pos.z, new_radius, layer);
    end
    ...
    local log = "radius="..radius.." angle="..angle.." pos={x="..PublicFunc.RoundNumber(pos.x,PublicStruct.Const.ROUND_NUMBER_BIT).." y="..PublicFunc.RoundNumber(pos.y,PublicStruct.Const.ROUND_NUMBER_BIT).." z="..PublicFunc.RoundNumber(pos.z,PublicStruct.Const.ROUND_NUMBER_BIT).."}".." GetOverlapSphere="..obj_cnt
    if angle ~= 360 and dir then
        log = log.." dir={x="..PublicFunc.RoundNumber(dir.x,PublicStruct.Const.ROUND_NUMBER_BIT).." y="..PublicFunc.RoundNumber(dir.y,PublicStruct.Const.ROUND_NUMBER_BIT).." z="..PublicFunc.RoundNumber(dir.z,PublicStruct.Const.ROUND_NUMBER_BIT).."}"
    end
    Root.lua_file_log(log)
    --[[local obj_list_name = ""
    for k, v in pairs(obj_list) do
        obj_list_name = obj_list_name.." "..v
    end
    Root.lua_file_log("obj_list="..obj_list_name)]]
    ...
    for k, v in pairs(obj_list) do
        ...
        if entity then
            ...
            Root.lua_file_log("cann't found obj")
    ...
    if #return_table > 0 then
        if PublicStruct.Fight_Sync_Type == ENUM.EFightSyncType.ServerFight or
            ...
            table.sort(return_table, sort_overlap_sphere)
        end
        local name = ""
        for i=1, #return_table do
            name = name.." "..return_table[i].name
        end
        Root.lua_file_log("return_table1="..name)
    end
    return return_table;
end
CT1 = 0;
...
--[[	COUNT = COUNT + 1;
    ...
    Root.lua_file_log("GetOverlapSphereRound="..obj_cnt)
    local return_table = { };
    ...
    if #return_table > 0 then
        ...
        Root.lua_file_log("return_table2="..name)
    end
...
function algorthm.GetOverlapRectangle(pos, direct, long, width, layer)
    ...
    local obj_list;
    if PublicStruct.Fight_Sync_Type == ENUM.EFightSyncType.Replay then
        obj_list = ObjectManager.OverlapSphere(pos.x, pos.y, pos.z, radius, layer);
    else
        obj_list = util.overlap_sphere(pos.x, pos.y, pos.z, radius, layer);
    end
    if not obj_list then return obj_list end
    local obj_cnt = 0;
    for k, v in pairs(obj_list) do
    ...
    Root.lua_file_log("GetOverlapRectangle cnt="..obj_cnt.." pos={x="..PublicFunc.RoundNumber(pos.x,PublicStruct.Const.ROUND_NUMBER_BIT).." y="..PublicFunc.RoundNumber(pos.y,PublicStruct.Const.ROUND_NUMBER_BIT).." z="..PublicFunc.RoundNumber(pos.z,PublicStruct.Const.ROUND_NUMBER_BIT).."} direct={x="..PublicFunc.RoundNumber(direct.x,PublicStruct.Const.ROUND_NUMBER_BIT).." y="..PublicFunc.RoundNumber(direct.y,PublicStruct.Const.ROUND_NUMBER_BIT).." z="..PublicFunc.RoundNumber(direct.z,PublicStruct.Const.ROUND_NUMBER_BIT).."} long="..PublicFunc.RoundNumber(long,PublicStruct.Const.ROUND_NUMBER_BIT).." width="..PublicFunc.RoundNumber(width,PublicStruct.Const.ROUND_NUMBER_BIT))
    local return_table = { };
    for k, v in pairs(obj_list) do
        local entity = ObjectManager.GetObjectByIid(v)
        if entity then
            ...
            -- app.log_error("name="..v:get_name().." pos.x="..pos.x.." pos.z="..pos.z.." direct.x=".. direct.x.." direct.z="..direct.z.." long="..long.." width="..width.." obj_x="..obj_x.." obj_z="..obj_z)
    ...
    -- app.log_error("***********")
    -- for i=1, #return_table do
    --    app.log_error("name="..return_table[i]:get_name())
    -- end
    -- app.log_error("***********")
    if #return_table > 0 then
        if PublicStruct.Fight_Sync_Type == ENUM.EFightSyncType.ServerFight or
        ...
        Root.lua_file_log("return_table3="..name)
    end
    return return_table;
end
...
function algorthm.Cross(x1, y1, z1, x2, y2, z2)
