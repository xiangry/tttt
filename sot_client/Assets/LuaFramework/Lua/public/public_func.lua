...
-- 参数:待分割的字符串,分割字符
-- 返回:子串表.(含有空串)
function PublicFunc.string_split(str, split_char, plain)
    local sub_str_tab = { };

    while (true) do
        local pos = string.find(str, split_char, 1, plain);
        if (not pos) then
...
function PublicFunc.obj_init(obj, pos)
    ...
    if pos then
        if not pos.x or not pos.y or not pos.z then
            app.log_error('调用公共函数PublicFunc.obj_init出错了，提供了pos参数但是内容是错的。' .. debug.traceback())
        end
...
function PublicFunc.isUnityPause()
    return app.get_time_scale() == 0;
end
...
function PublicFunc.IdToConfig(id)
    ...
    elseif PropsEnum.IsVaria(id) then
        ...
        app.log_error("id:" .. tostring(id) .. "没有对应的配置表可读");
    end
    return info;
end
...
function PublicFunc.GenerateSkillTargetIndex(info, skill_id, buff_id, trigger_index, action_index, action_ref)
    ...
    if skill_id == nil or buff_id == nil or trigger_index == nil or action_index == nil or action_ref == nil then
        app.log_error("组合targets_inde出现问题".." skill_id="..tostring(skill_id).." buff_id="..tostring(buff_id).." trigger_index="..tostring(trigger_index).." action_index="..tostring(action_index).." action_ref="..action_ref.." "..debug.traceback())
    end
...
function PublicFunc.SetIconFrameSprite(sprite, rarity_num)
    if sprite == nil or type(rarity_num) ~= "number" or rarity_num > 6 or rarity_num < 0 then
        -- app.log_error('SetIconFrameSprite sprite == nil' .. debug.traceback())
        return
    end
...
function PublicFunc.SetStarBreakIcon(sprite, lvl)
    if sprite == nil or type(lvl) ~= "number" then
        return
    end
    local index = math.ceil(lvl/9)
    local spName = EStarBreakIcon[index]
    if spName then
        sprite:set_sprite_name(spName)
    end
end
...
function PublicFunc.msg_regist(msg, callback)
    ...
    if g_open_msg_regist then
        ...
        --     app.log_error("1...."..tostring(callback))
        -- end
    end
...
function PublicFunc.msg_unregist(msg, callback)
    ...
    -- app.log_error("注销"..#PublicFunc.wait_unregist_list);
...
function PublicFunc.UnregistAllMsg()
    if PublicFunc.wait_unregist_list then
        for k, v in pairs(PublicFunc.wait_unregist_list) do
            ...
            if g_open_msg_regist then
                -- if msg == "stop_skill_cd" then
                --     app.log_error("2...."..tostring(callback))
                -- end
...
function PublicFunc.SceneWorldPosToUIWorldPos(x, y, z)
    ...
    if sceneCamera ~= nil and uiCamera ~= nil then
        ...
        app.log_error('SceneWorldPosToUIWorldPos camera == nil or ui_camera == nil')
    end
...
function PublicFunc.SceneWorldPosToUIScreenPos(x, y, z)
    ...
    if uiCamera ~= nil then
        ...
        app.log_error('SceneWorldPosToScreenPos camera == nil')
    end
...
function PublicFunc.SetCardInfoForUi(tCls,cardInfo)
    if type(tCls) ~= "table" or nil == cardInfo then
        app.log_error("PublicFunc.SetCardInfoForUi 参数类型错误,"..type(tCls).."  "..type(cardInfo))        
        return
    end
    --名称
...
function PublicFunc.SetSkillLock(cardHuman, obj, index)
    -- app.log_error(tostring(cardHuman.learn_skill[index])..".."..index.."..."..tostring(cardHuman.rarity).."..."..tostring(obj));
    local show = false;
...
function PublicFunc.NumberToStringByCfg(number)
    local str;    
    local flg;
    local cfg = ConfigManager.Get(EConfigIndex.t_number_to_str_snk, AppConfig.get_update_region());
...
function PublicFunc.UpdateConfigHeroInfo(info, config, config_scale)
    ...
    local skill1_level = 1
    local skill2_level = 1
    local skill3_level = 1
    if type(config.skill_level) == "table" then
        skill1_level = config.skill_level[1] or 0
        skill2_level = config.skill_level[2] or 0
        skill3_level = config.skill_level[3] or 0
    end
    table.insert(data.learn_skill, {id=cardConfig.spe_skill[1][1], level=skill1_level})
    table.insert(data.learn_skill, {id=cardConfig.spe_skill[2][1], level=skill2_level})
    table.insert(data.learn_skill, {id=cardConfig.spe_skill[3][1], level=skill3_level})

    local skill4_level = config.skill4_level or 1
    local skill5_level = config.skill5_level or 1
    local skill6_level = config.skill6_level or 1
    ...
    if passiveConfig and #passiveConfig >= 3 then
        ...
        table.insert(data.passivity_property_info, {id=passiveConfig[1].skill, level=skill4_level})
        table.insert(data.passivity_property_info, {id=passiveConfig[2].skill, level=skill5_level})
        table.insert(data.passivity_property_info, {id=passiveConfig[3].skill, level=skill6_level})
    end
...
function PublicFunc.contain_quotation_marks(str)
	local ret1 = string.find(str, '\'')
	if ret1 then
		return true
	end
	ret1 = string.find(str, '\"');
	if ret1 then
		return true
	end
	return false
...
-- breif:   获取ui相机中的gameobject绝对坐标值
-- param    x,y,z 为gameobject的世界坐标值
function PublicFunc.WorldToScreenNguiPoint(x, y, z)
...
function PublicFunc.TouchPtToNguiPoint(x, y)
    ...
    -- 宽屏UI不满屏
    if screen_width / screen_height >= 1.8 then
        local logic_screen_width = screen_height * 16 / 9
        local scale = 1280 / logic_screen_width
        local x1 = x * scale
        local y1 = y * scale;
        local ngui_x = x1 - 640 - (screen_width - logic_screen_width) * scale / 2;
        local ngui_y = y1 - 640 * screen_height / logic_screen_width;
...
function PublicFunc.FuncIsOpen(id, in_level)
    ...
    if unlockData then
        local level = in_level or DataCenterGet(ESnkDataCenter.player):GetLevel()
        if level >= unlockData.open_level then
            --签 到 处理
            if id == MsgEnum.eactivity_time.eActivityTime_checkinMonth then
                if g_dataCenter.activityReward:GetSignIn30RedPoint() then
                    return true
                else
                    return false
                end
            -- elseif id == MsgEnum.eactivity_time.eActivityTime_LoginGift then
            --     if PublicFunc.GetSignIn8Tip() then
            --         return false
            --     else
            --         return true
            --     end
...
function PublicFunc.GetSignIn8Tip()
    local index = 0
    for i=1,8 do
        local data = g_dataCenter.activityReward:getReward(MsgEnum.eactivity_time.eActivityTime_loginReward,i-1)
        if data then
    ...
    app.log_error("GetSignIn8Tip....."..tostring(index))
    if index == 8 then
        return true
    else
        return false
...
function PublicFunc.GetStarUpSoulId(role_number)
    --app.log("PublicFunc.GetStarUpSoulId:"..role_number)
    ...
    if role_index_cfg then
        for k, v in pairs(ConfigManager._GetConfigTable(EConfigIndex.t_item)) do
            if v.hero_number == role_index_cfg.default_rarity and v.category == 6  then
                return k
            end
        end
    else
        app.log_error("PublicFunc.GetStarUpSoulId role_index_cfg:  " .. role_number)
    end
    return nil
end

function PublicFunc.GetSoulIcon(number)
    local cfg = ConfigManager.Get(EConfigIndex.t_role_index, number)
    if cfg then
        for _, v in pairs(ConfigManager._GetConfigTable(EConfigIndex.t_item)) do
            if v.category == 6 and v.hero_number == cfg.default_rarity then
                return v.small_icon
            end
        end
    end
...
local EButtonData = {
    [1] = {nameSprite="snk_ui_common_0_new_but_b_3", colorTxt={151,57,0}},        --黄色
    [2] = {nameSprite="snk_ui_common_0_new_but_b_2", colorTxt={60,75,143}},       --紫色
    [3] = {nameSprite="snk_ui_common_0_new_but_b_3", colorTxt={198,198,198}},     --灰色
}
--设置通用按钮显示模式 1:黄色 2:蓝色 3:灰色
function PublicFunc.SetButtonShowMode(btn, mode, spName, labName, desc)
    ...
    if cfg_name then
        ...
        if skillLevelData then
            if not info.halo_level or info.halo_level == 0 then
                table.insert(result, {id=1, level=1, enable=false, skillType=2});
            else
...
function PublicFunc.FormatSkillString(str, skill_id, old_lv, new_lv, atk_power)
    ...
    if old_lv and skill_id then
        local skillCfg = ConfigManager.Get(EConfigIndex.t_skill_info, skill_id)
        if skillCfg then
            local skillInfo = ConfigManager.Get(EConfigIndex.t_skill_level_info, skillCfg.level_map_id)
    ...
    if not source_str then
        app.log_error("找不到技能字符串。skillid："..skill_id);
        return "",0;
    end
    local _,_,attr_type,param = string.find(source_str, "#([defs])%-([%w_]+)#");

    --app.log("attr_type........"..table.tostring(attr_type))
    --app.log("param............"..tostring(param))
...
function PublicFunc.FormatPassiveSkillString(str, skill_id, role_id, old_lv, new_lv)
    ...
    if not source_str then
        app.log_error("找不到技能字符串。skillid："..skill_id.." role_id:"..role_id);
        return "",0;
    end
    local _,_,attr_type,param = string.find(source_str, "#([defs])%-([%w_]+)#");
    local value = 1;
...
function PublicFunc.FormatHaloSkillString(str, skill_id, role_id, old_lv, new_lv)
    local source_str = str
    local old_data, new_data
    --app.log_error("找不到技能字符串。skillid："..skill_id.." role_id:"..role_id.. " lv:"..new_lv);
    if old_lv and skill_id then
...
function PublicFunc.FormatHetiSkillString(str, skill_id, role_id, old_lv, new_lv)
    ...
    if skill_id then
        ...
        if skillCfg then
            local skillInfo = ConfigManager.Get(EConfigIndex.t_skill_level_info, skillCfg.level_map_id)
            if skillInfo then
                old_data = skillInfo[1]
            end
        end
    end

    app.log("old_data.........."..table.tostring(old_data))

    local source_str = str

    local _,_,attr_type,param = string.find(source_str, "#([defs])%-([%w_]+)#");

    --app.log("attr_type........"..table.tostring(attr_type)) GetPropertyVal
    app.log("param............"..tostring(param))
    ...
    for k,v in pairs(hero_list) do
        ...
        all_atk = all_atk + obj:GetPropertyVal(ENUM.EHeroAttribute.atk_power)
        all_def = all_def + obj:GetPropertyVal(ENUM.EHeroAttribute.def_power)
    end

    
    app.log("all_atk..........."..tostring(all_atk))
    app.log("all_def..........."..tostring(all_def))
   ...
   -- do return end
    ...
    while attr_type do
        if attr_type == "d" then
            ...
            if old_data and type(old_data.damage) == "table" then
                old_attr = old_data.damage[tonumber(param)] or {};
                app.log("old_attr......"..table.tostring(old_attr))
                old_atkscale = old_attr.atkscale or 0;
                old_fixeddamage = old_attr.fixeddamage or 0;
            end
            local old_value = all_atk * old_atkscale
            local new_value = old_fixeddamage
            value = new_value + old_value;
...
function PublicFunc.GetPropertyIdNamePair(isShow)    
    ...
    -- app.log_error("=== config "..table.tostring(config))
...
-- 返回 {{show_name,show_value,property_id}, ...}
function PublicFunc.GetPropertyShowValue(info)
    local result = {}

    if info then
        local show_name, show_value = nil, nil
        for property_id, property_name in pairs_key(PublicFunc.GetPropertyIdNamePair(true)) do
            show_name = gs_string_property_name[ property_id ]
            ...
            --     if halo_info and halo_info.halo_property and halo_info.halo_property[ENUM.EHeroAttribute[property_name]] then
            --         add_property = halo_info.halo_property[ENUM.EHeroAttribute[property_name]]
            --     end
            -- end
            show_value = PublicFunc.GetPropertyValueForamt(property_id, info:GetPropertyVal(property_name)+add_property)
            table.insert(result, { show_name, show_value, property_id })
        end
    end
...
function PublicFunc.GetPropertyValueForamt(property_id, value, return_number_value)
    local eValueType = ENUM.EAttributeValueType[property_id]
    if  eValueType then
    ...
    if return_number_value == true then
        return value
    else
...
function PublicFunc.BuyCheck(cbFunc, cbParam, costCount, costType)
    if cbFunc == nil or costCount == nil then 
		app.log_error("*** PublicFunc.BuyCheck 参数错误"..debug.traceback())
		return;
	end

	local costType = costType or IdConfig.Crystal
	local ownCount = DataCenterGet(ESnkDataCenter.player):GetCrystal() or 0
...
function PublicFunc.GotoRecharge()
    HintUI.SetAndShowNew(EHintUiType.two, 
        "钻石不足", "钻石不足！是否前往充值？", 
        nil,
        {func=function() uiManager:PushUi(EUI.SnkStoreUI) end, str="立即前往"},
        {str="取消"} );
end

function PublicFunc.ExchangeGold(needGold)
    if needGold == nil then
        app.log_error("*** PublicFunc.ExchangeGold 参数错误"..debug.traceback())
        return
    end
...
function PublicFunc.GetRarityInfo(level)
    ...
    if rarity_name then
        ...
        if _dd then
            ...
            app.log_error(string.format("#PublicFunc.GetRarityInfo# rarity_name=%s name=%s level=%s",tostring(rarity_name),tostring(name),tostring(level)))
        end
        ...
        --app.log_error("PublicFunc.GetRarityInfo failed "..tostring(level))         
        return data["White"] 
    end
end
...
function PublicFunc.GetHeroRarityStr(rarity)
    ...
    return __heroRarityString[rarity] or tostring(rarity)
end
...
function PublicFunc.GetContactDescData(roleData)
    if roleData == nil then
        app.log_error("role data is nil!")
        return
    end
...
function PublicFunc.CreateSendTeamData(teamid, teamData)
    if type(teamid) == "table" then
        ...
        for i, v in ipairs(teamid) do
            teamList[i] = {}
            teamCardsList[i] = {}

            local cardsList = {}
            for j=1, 3 do
                cardsList[j] = teamData[(i-1) * 3 + j] or "0" -- 服务器记录了空位置信息
            end
            teamList[i].teamid = v or 0
            teamList[i].cards = cardsList
        end
...
function PublicFunc.FloatErrorCode(rst, error_table, table_name)
    ...
    if str == nil then
        app.log_error("rst="..tostring(rst).." error_table="..table.tostring(error_table))
...
function PublicFunc.GetDiscreteConfig(id, default_value)
    ...
    if data then
        ...
        app.log_error("error discrete_id:"..tostring(id))
        ...
        if param then
            ...
            if param.ai ~= nil then
                ...
                if ConfigManager.Get(EConfigIndex.t_hfsm,aiid) == nil then
                    app.log_error("mapinfo config ai id error! id = " .. tostring(aiid))
                else
...
function PublicFunc.ConstructCardAndSort(items)
    for k,item in ipairs(items) do
        ...
        elseif PropsEnum.IsItem(item.id) or PropsEnum.IsVaria(item.id) then
            ...
            app.log_error("dont support equipment!")
            return items
        end
    end
...
function PublicFunc.ProcessNameSplit(name)
    if name == nil then
        app.log_error("nil name from PublicFunc.ProcessNameSplit "..debug.traceback())
        return
    end
...
function PublicFunc.GetHeroNextProperyDiff(roleData, nextNumber, pList, isStarUp)
    local propertyDiff = {} 
    ...
    --连协
    local currContactProperty = PublicFunc._GetContactProperyTotal(roleData.number, roleData.level, pList, currHeroProperty)
    local nextContactProperty = PublicFunc._GetContactProperyTotal(nextNumber, roleData.level, pList, nextHeroProperty)
    local propertyPer = PublicFunc._GetContactProperyPer(roleData)
    --被动技能
    local passivePropertyDiff = {}
    if isStarUp then
        passivePropertyDiff = PublicFunc.GetPassivePropertyDiff(roleData, roleData.rarity + 1)
    end

    for k, v in pairs(pList) do
        local contactDiff = 0
        if propertyPer[v] then
            contactDiff = (nextContactProperty[v] - currContactProperty[v]) * propertyPer[v] / 10000
        end
        local _activeV = _activeProperty[v] or 0
        local _passiveDiff = passivePropertyDiff[v] or 0
        propertyDiff[v] = nextHeroProperty[v] + _activeV - currHeroProperty[v] + contactDiff + _passiveDiff
    end
    return propertyDiff
end

function PublicFunc.GetHeroPreProperyDiff(roleData, preNumber, pList, isStarUp)
    local propertyDiff = {} 
    local currHeroProperty = PublicFunc._GetHeroProperyTotal(roleData.number, roleData.level, pList)
    ...
    --连协
    local currContactProperty = PublicFunc._GetContactProperyTotal(roleData.number, roleData.level, pList, currHeroProperty)
    local preContactProperty = PublicFunc._GetContactProperyTotal(preNumber, roleData.level, pList, preHeroProperty)
    local propertyPer = PublicFunc._GetContactProperyPer(roleData)
    --被动技能
    local passivePropertyDiff = {}
    if isStarUp then
        passivePropertyDiff = PublicFunc.GetPassivePropertyDiff(roleData, roleData.rarity)
    end

    for k, v in pairs(pList) do
        local contactDiff = 0
        if propertyPer[v] then
            contactDiff = (currContactProperty[v] - preContactProperty[v]) * propertyPer[v] / 10000
        end
        local _activeV = _activeProperty[v] or 0
        local _passiveDiff = passivePropertyDiff[v] or 0
        propertyDiff[v] = currHeroProperty[v] + _activeV - preHeroProperty[v] + contactDiff + _passiveDiff
    end
    return propertyDiff
end
...
function PublicFunc._GetHeroProperyTotal(number, level, pList)
    ...
    local propertyValue = {} 
    for k, v in pairs(pList) do
        local _roleV = roleConfig[v] or 0
        --全能值无
        local _roleLevelV = roleLevelConfig[v] or 0
        propertyValue[v] = _roleV + _roleLevelV
    end
...
function PublicFunc.GetPassivePropertyDiff(roleData, star)
    local nextCfg = ConfigManager.Get(EConfigIndex.t_role_skill, star)
    local property = {
        ["max_hp"] = 0,
        ["atk_power"] = 0,
        ["def_power"] = 0
    }
    local diffOpenId = nil
    if nextCfg then
        --只计算增量
        local nextOpenId = nextCfg.passive_property
        if nextOpenId ~= 0 then
            if type(nextOpenId) == "table" then
                diffOpenId = nextOpenId[#nextOpenId]
            end
        end
    end
    if diffOpenId then
        local cfg_name = EConfigIndex["t_passive_property_" .. roleData.default_rarity]
        local cfg = ConfigManager.Get(cfg_name, diffOpenId)
        if cfg then
            for _, v in pairs(cfg) do
                if v.level == 1 then
                    property["max_hp"] = v.max_hp
                    property["atk_power"] = v.atk_power
                    property["def_power"] = v.def_power
                    break
                end
            end
        end
...
--[[取一星属性  去掉百位编号]]
function PublicFunc.GetOneStarNumber(number)
    --s_int32 oneStarNumber = get_number() / 1000 * 1000 + get_number() % 100;
    return math.floor(number/1000) * 1000 + (number - math.floor(number/100) * 100)
end

function PublicFunc._GetContactProperyTotal(number, level, pList, defaultProperty)
    local newNumber = PublicFunc.GetOneStarNumber(number)
    if newNumber == number then
        return defaultProperty
    end
    return PublicFunc._GetHeroProperyTotal(newNumber, level, pList)
end

--[[连协加成]]
function PublicFunc._GetContactProperyPer(roleData)
    local contactConfig = ConfigManager.Get(EConfigIndex.t_role_contact, roleData.default_rarity)
    local propertyPer = {
        ["max_hp"] = 0,
        ["atk_power"] = 0,
    ...
    for _, v in ipairs(contactConfig) do
        ...
        if isActive then
            propertyPer["max_hp"] = propertyPer["max_hp"] + v.hp_per
            propertyPer["atk_power"] = propertyPer["atk_power"] + v.attack_per
            propertyPer["def_power"] = propertyPer["def_power"] + v.def_per
        end
    end
    return propertyPer
end
...
-- 两个英雄是否有克制关系
function PublicFunc.IsRestraintBetweenTwoHero(cardNumber1, cardNumber2)
    local result = nil
    if not cardNumber1 or not cardNumber2 then
        return result
    end

    local cfg1 = ConfigHelper.GetRole(cardNumber1)
    local cfg2 = ConfigHelper.GetRole(cardNumber2)
    if cfg1 and cfg2 and cfg1.pro_type ~= cfg2.pro_type then
        local config = ConfigManager._GetConfigTable(EConfigIndex.t_role_restraint)
        -- 1,2,3  防,攻,技
        local restraint_plus_name_to_protype = {
            restraint_def_plus_per = 1,
            restraint_attack_plus_per = 2,
            restraint_skill_plus_per = 3,
        }
        local data = config[cfg1.pro_type]
        for k, v in pairs(restraint_plus_name_to_protype) do
            if v == cfg2.pro_type and data[k] > 0 then
                result = true
                break
            end
        end
        if result == nil then
            data = config[cfg2.pro_type]
            for k, v in pairs(restraint_plus_name_to_protype) do
                if v == cfg1.pro_type and data[k] > 0 then
                    result = false
                    break
                end
            end
...
function PublicFunc.CheckCanUseNavMeshOperation(obj, logError)
    local isShow = obj:IsShow()
    local inhiIsShow = obj:InHierarchyIsShow()
    local canUse = isShow and inhiIsShow
    if canUse then
    ...
    if logError and not canUse then
        ...
        local log = "CheckCanUseNavMeshOperation is false " .. obj:GetName() .. ' ' .. tostring(isShow) .. ' ' .. tostring(inhiIsShow) .. ' State:[' .. obj:GetObjHFSMState() .. ']' .. debug.traceback()
...
function PublicFunc.YanjiusuoInteger(nNum)
    local n = 1;
...
function PublicFunc.GetSkillCfg(def_rarity, skill_type, skill_id)
    ...
    elseif skill_type == 3 then
        ...
        skillData = ConfigManager.Get(EConfigIndex.t_skill_info,skill_id); 
        if skillData then
            skillLevelData = ConfigManager.Get(EConfigIndex.t_skill_level_info,skillData.level_map_id);
        end
    end
...
function PublicFunc.MediaPlay(file_path, finish_func, ready_func, is_pause_ready, is_skip, stop_audio_delay, is_part)
    ...
    local _error_func = function()
        app.log_error("视频播放失败："..tostring(file_path))
    end
...
function PublicFunc.SetScaleByScreenRate(obj)
    if obj == nil then
        app.log_error('obj == nil' .. debug.traceback())
        return
    end
...
--领取奖励的时候做体力上限检查
function PublicFunc.CheckGetAwardHaveAP(awards,okFunc,tip1,tip2)
	local haveAp = false;
	local awardApNum = 0;
	for k,v in pairs(awards) do 
		if tonumber(v.item_id) == 4 then 
			haveAp = true;
			awardApNum = awardApNum + v.item_num;
		end
	end 
	if haveAp then 
		local curAp = g_dataCenter.player.ap;
		local maxAp = ConfigManager.Get(EConfigIndex.t_discrete,83000165).data;
		if maxAp > curAp and maxAp < curAp + awardApNum then 
			HintUI.SetAndShowNew(EHintUiType.two,"提示",tip1,nil,{str="确定",func = okFunc},{str = "取消"})
		elseif maxAp <= curAp then 
			HintUI.SetAndShowNew(EHintUiType.two,"提示",tip2,nil,{str="确定",func = okFunc},{str = "取消"})
		else 
			okFunc();
		end
	else 
...
function PublicFunc.GetEquipExpertProps(heroId, level, type)
    ...
    if not cfgData then
        app.log_error('无英雄配置数据！ heroId = ' .. tostring(heroId) .. '  level = ' .. tostring(level))
        return
    end
    ...
    local retProps = {}
    for i = ENUM.EHeroAttribute.cur_hp, ENUM.EHeroAttribute.def_pierce do
        local _key = ENUM.EHeroAttributeKey[i]
        local prop = prefixCfg[type] .. _key
        if cfgData[prop] and cfgData[prop] ~= 0 then
            table.insert(retProps, {key = _key, value = PublicFunc.AttrInteger(cfgData[prop])})
        end
    end
    return retProps
end
...
function PublicFunc.GetHeroPropertyName(prop)
    if prop == nil then
        app.log_error("prop == nil")
        return ""
    end
    local propId = ENUM.EHeroAttribute[prop]
    if propId and gs_string_property_name[propId] then
        return gs_string_property_name[propId]
    end
    return ""
end

function PublicFunc.NeedAttributeVerify(play_method_type)
    if PublicStruct.Fight_Sync_Type == ENUM.EFightSyncType.ServerFight or
        PublicStruct.Fight_Sync_Type == ENUM.EFightSyncType.Replay then
        return false
    end
...
function PublicFunc.IsSpecEquipLevelLock(position)
    ...
    if id ~= nil then
        ...
        return DataCenterGet(ESnkDataCenter.player):GetLevel() < level, level
    end
    return false
end
...
--立绘兑换活动
function PublicFunc.IsBagShowLookBtn(itemid)
    local res = false
    if itemid then
        local exchangeData = ConfigManager.Get(EConfigIndex.t_item_exchange,itemid)
        if exchangeData and exchangeData.type == 7 then
            res = true
        end
    end
    return res
...
local __teamNameStr = 
{
    ...
    [ENUM.ETeamType.normal_2] = gs_misc['team_normal2'],

    [ENUM.ETeamType.kuikuliya] = gs_misc['team_jixiantiaozhan'],
    [ENUM.ETeamType.kuikuliya_2] = gs_misc['team_jixiantiaozhan2'],

    [ENUM.ETeamType.arena] = gs_misc['team_arena'],
    [ENUM.ETeamType.arena_2] = gs_misc['team_arena2'],

    [ENUM.ETeamType.world_boss1] = gs_misc['team_world_boss'],
    [ENUM.ETeamType.world_boss2] = gs_misc['team_world_boss2'],

    [ENUM.ETeamType.world_treasure_box] = gs_misc['team_treasure_box'],

    [ENUM.ETeamType.guild_gaosujuji] = gs_misc['team_gaosujuji'],
    [ENUM.ETeamType.guild_gaosujuji_2] = gs_misc['team_gaosujuji2'],

    [ENUM.ETeamType.guild_Defend_war] = gs_misc['team_defend_war'],
    [ENUM.ETeamType.guild_Defend_war_2] = gs_misc['team_defend_war2'],

    [ENUM.ETeamType.Clown_plan] = gs_misc['team_clown_plan'],
    [ENUM.ETeamType.Clown_plan_2] = gs_misc['team_clown_plan2'],
}
...
function PublicFunc.CheckAcquiringWayEnable(itemId)
    local itemCfg = ConfigManager.Get(EConfigIndex.t_item, itemId)
    if itemCfg == nil or itemCfg.acquiring_way == nil or type(itemCfg.acquiring_way) ~= "table" then
        return false
    end
    local level = DataCenterGet(ESnkDataCenter.player):GetLevel()
    for k, v in pairs(itemCfg.acquiring_way) do
        local cfg = ConfigManager.Get(EConfigIndex.t_acquiring_way_ui_data, v)
        if cfg then
            if cfg.destype == 1 then
                local openLevel = ConfigManager.Get(EConfigIndex.t_hurdle, cfg.system_id).need_level
                if level >= openLevel then
                    if g_dataCenter.hurdle:IsCanFight(cfg.system_id) then
                        return true
                    end
                end
            else
                local openLevel = ConfigManager.Get(EConfigIndex.t_play_vs_data, cfg.system_id).open_level
                if level >= openLevel then
                    return true
                end
            end
...
function PublicFunc.getHetiSkillId(num)
    local id = 0
    if num == 1 then
        id = 10
    elseif num == 2 then
    ...
    if id == 0 then
        app.log("人物攻击属性错语，请检查配置表")
    end 

    return id
end

function PublicFunc.IsOpenHetiSKillRushCD(skillid)

    local flag = false
    local hetiskillinfo = ConfigManager.Get(EConfigIndex.t_combination_skill_info,skillid)
    
    if hetiskillinfo then
        flag = true
    end
...
function PublicFunc.IsOpenHetiSKill()
    if PublicFunc.IsKeyFrameSyncType() then
        return true
    end
    local flag = false
    local openLevel = ConfigManager.Get(EConfigIndex.t_discrete,83000231).data

    local nowLevel = DataCenterGet(ESnkDataCenter.player):GetLevel()
...
function PublicFunc.RoundNumber(number, decimals)
    if number == nil then
        return 0.14141
    end
    local add,scale,cy
...
function PublicFunc.IsKeyFrameSyncType()
    if PublicStruct.Fight_Sync_Type == ENUM.EFightSyncType.Replay or 
        PublicStruct.Fight_Sync_Type == ENUM.EFightSyncType.ServerFight then
        return true
    end
	return false;
end

function PublicFunc.IsReplayMyPlayerid(replayPlayerid)
    if replayPlayerid then
        local playerid = tostring(DataCenterGet(ESnkDataCenter.player).playerid)
        return string.find(replayPlayerid, playerid) == 1
    else
        return false
    end
end
...
-- breif:   构建离线战斗card数据
-- param    cardInfo:   数据源
-- param    svrStruct:  数据源是否是服务器数据结构
-- return   返回战斗card数据，数据源为空时也会返回1个数据（dataid = 0）
function PublicFunc.CreateOfflineFightCardInfo(cardInfo, svrStruct)
    local card_info = {}
    if cardInfo then
        card_info.config_id = cardInfo.number
        card_info.level = cardInfo.level
        card_info.dress_id = cardInfo.dressed_fashion_clothes
        
        if svrStruct then
            card_info.data_id = cardInfo.dataid
            card_info.skills = cardInfo.skill_info
            card_info.propertys = {}
            for i=1, #cardInfo.property do
                if cardInfo.property[i] ~= 0 then
                    table.insert(card_info.propertys, {type=i-1, value=cardInfo.property[i]})
                end
            end
        else
            card_info.data_id = cardInfo.index
            card_info.skills = cardInfo.learn_skill
            card_info.propertys = {}
            for k, v in pairs(cardInfo.property) do
                if v ~= 0 then
                    table.insert(card_info.propertys, {type=k-ENUM.min_property_id-1, value=v})
                end
            end
        end
    else
        card_info.config_id = 0
        card_info.dress_id = 0
        card_info.level = 0
        card_info.data_id = 0
        card_info.propertys = table.empty()
        card_info.skills = table.empty()
    end

    return card_info
end

function PublicFunc.GetDirection(sx, sy, sz, tx, ty, tz)
    local dx,dy,dz,success
    if sx == tx then
        dx = 0
    else
        dx = tx - sx
    ...
    dx,dy,dz,success = algorthm.Normalized(dx,dy,dz);
    return dx,dy,dz,success
end

--官网 开启实名
function PublicFunc.IsOpenReName()
...
--官网 安卓 开启分享
function PublicFunc.isOpenShare()
    if Root.get_os_type() ~= 8 and AppConfig.get_check_digisky_channel() then
        return true
    else
        return false
    end
...
function PublicFunc.IsStarBreakOpen()
    local cfg = ConfigManager.Get(EConfigIndex.t_discrete, MsgEnum.ediscrete_id.eDiscreteID_hero_star_break_is_open)
    if cfg then
        return cfg.data == 1
    end
    return false
...
function PublicFunc.GetDressIdByRoleNumber(number)
    return 0
    --local rarityNumber = CardHuman.GetDefaultRarityNumber(number)
    --local card = g_dataCenter.package:find_card_for_num(ENUM.EPackageType.Hero, rarityNumber);
    --local dressId = 0
    --if card then
    --    dressId = card.dressed_fashion_clothes
    --end
    --return dressId
end

--替换'[]', 处理如'[ff0000] ...'
function PublicFunc.GetShowStr(str)
    local ret = string.gsub(str, "%[", "")
    return string.gsub(ret, "]", "")
end

function PublicFunc.ConverStringIDToString(idstr)
    local res = idstr
    if string.find(idstr, "gs_") then
        local ids = PublicFunc.string_split(PublicFunc.trim(idstr), '.', true)
        local fileAllString = _G[ids[1]]
        if fileAllString then
            res = fileAllString[ids[2]]
        end
    end
...
function PublicFunc.IsOpenSecondTeam()
    return #(g_dataCenter.package:get_hero_card_table()) > 3
end

function PublicFunc.GetHeTiIconName(pro_type)
    if type(pro_type) ~= "table" then
        return "assetbundles/prefabs/ui/image/icon/he_ti_ji/ht_wenhao.assetbundle", "未激活";
    end
    local hetiskillid = 0;
    for k, v in pairs(pro_type) do
        hetiskillid = hetiskillid + PublicFunc.getHetiSkillId(v);
    end
    local hetiskillinfo = ConfigManager.Get(EConfigIndex.t_combination_skill_info, hetiskillid);
    if hetiskillinfo == nil then
        return "assetbundles/prefabs/ui/image/icon/he_ti_ji/ht_wenhao.assetbundle", "未激活";
    end
    return hetiskillinfo.icon, hetiskillinfo.name;
end

--[[加载配置]]
function PublicFunc.LoadFileSetting(fileName)
    if file.exist(fileName) == true then
        local fileHandle = file.open_read(fileName)
...
--[[保存配置]]
function PublicFunc.SaveFileSetting(fileName, setting)
    local fileHandle = file.open(fileName, 2);
    fileHandle:write_string(table.toLuaString(setting));
    fileHandle:close();
end
...
function PublicFunc.GetSexText(sex)
    return _SexText[sex];
end

-- 角色助战描述
function PublicFunc.GetRoleHelpText(default_rarity)
    local result = ""
    local config = ConfigManager.Get(EConfigIndex.t_role_index, default_rarity)
    if config then
        result = tostring(config.help_effect_text)
    else
        local roleConfig = ConfigHelper.GetRole(default_rarity)
        if roleConfig and roleConfig.default_rarity then
            config = ConfigManager.Get(EConfigIndex.t_role_index, roleConfig.default_rarity)
            result = tostring(config.help_effect_text)
        end
    end
    return result;
...
function PublicFunc.IsObstacle(obj)
    local ai = nil
    if obj and obj.config then
        ai = obj.config.ai
    end

    return ai == 131 or ai == 144 or ai == 150
end

local _gameID = {
    [1] = ENUM.Activity.activityType_little_game_qts,
    [2] = ENUM.Activity.activityType_little_game_light,
}
function PublicFunc.GetLittleGameID()
    return _gameID
end

function PublicFunc.CheckClassName(dataObj, className)
    if type(dataObj) == "table" and dataObj._className == className then
        return true
    end
    return false
end